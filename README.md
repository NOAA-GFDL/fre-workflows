...
Through the use of fre-cli tools ([`fre-cli repo`](https://github.com/NOAA-GFDL/fre-cli)), the fre-postprocess-workflow repository is checked out under and experiment name in `~/cylc-src/`, configurations needed for the workflow are created and validated, and the experiment is installed and run by cylc. Once run, cylc automates batch job submission to system resources and produces post-processing model output.
...
| ```environment.yml```             | Conda environment yaml file for installing `fre-postprocess-workflow` |
...
Please see [for-developers.md](https://github.com/NOAA-GFDL/fre-postprocess-workflow/blob/main/for-developers.md) for instructions
on how to run the postprocessing workflow as a workflow developer.

## Containerized Post-Processing Instructions

Please see
[portability-instructions.md](https://github.com/NOAA-GFDL/fre-postprocess-workflow/blob/main/portability-instructions.md) for
