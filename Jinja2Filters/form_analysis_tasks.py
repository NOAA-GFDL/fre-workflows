...
def form_analysis_tasks(pp_components, pp_dir):
...
"""
    ...
    for keys, sub_node in node.walk():
        ...
        item_chunk = node.get_value(keys=[item, 'chunk'])
        ...
        """.format(item=item, chunk=item_chunk, comp=item_comps[0], freq=item_freq, pp_dir=pp_dir)

    return(results)

#print(form_analysis_tasks('all', '/archive/Chris.Blanton/am5/2022.01/c96L33_am4p0_cmip6Diag/gfdl.ncrc4-intel21-prod-openmp/pp'))
