...
def test_regrid_xy(capfd, tmp_path):
       ...
       #  print(' '.join(ex))
        ...
        ex = [ 'fregrid', '--standard_dimension', '--input_mosaic', f'{input_mosaic}/{input_mosaic_file}',
               '--input_dir', f'{input_dir}', '--input_file', f'{fregrid_input_file}', '--associated_file_dir',
               f'{input_dir}', '--remap_file', f'{remap_dir}/{fregridRemapFile}', '--nlon', f'{nlon}', '--nlat',
               f'{nlat}', '--scalar_field', f'{vars}','--output_file', f'{output_dir}/{outputFile}' ]
        
        #TEMP DEBUG
        print("FREGRID CALL IS:")
        print(' '.join(ex))
        #TEMP DEBUG
...
def test_nccmp_regrid_xy(capfd, tmp_path):
    ...
    nccmp = [ 'nccmp', '-m', '--force', str(dr_remap_out / inputGrid / inputRealm / sources_xy / fregridRemapFile), str(remap_dir_out / fregridRemapFile) ];
    print (' '.join(nccmp));
