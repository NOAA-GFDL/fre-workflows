...
"""
analysis_validator.py (rose) validates the DO_ANALYSIS and DO_ANALYSIS_ONLY specifications rose-suite.conf, 
and the corresponding dependencies and requirements.
"""
...
class Analysis_Validator(metomi.rose.macro.MacroBase):
    ...
    def validate(self, config, meta_config=None):
        '''
        Takes in the config accessible via rose-suite.conf in main and opt,
        Return a list of errors, if any upon validation
        '''
        ...
        # this is a "i do not want analysis" and so should not error anything, so no report
        ...
        if do_analysis:
            ...
            if analysis_dir is not None:
                ...
                if os.access(analysis_dir.strip('"\''), os.W_OK):
                    ...
                    self.add_report('template variables', "ANALYSIS_DIR", analysis_dir,
                                    "ANALYSIS_DIR must exist and be writable if DO_ANALYSIS is set")
            else:
                self.add_report('template variables', 'ANALYSIS_DIR', analysis_dir,
                                "Must exist and be writable if DO_ANALYSIS is set")
        ...
        if fre_analysis_home is not None:
            if os.access(fre_analysis_home.strip('"\''), os.R_OK):
                ...
                self.add_report('template variables', "FRE_ANALYSIS_HOME", fre_analysis_home,
                                "FRE_ANALYSIS_HOME must be readable if set")

        # Validation: This snippet checks if there freq, script and comp for every analysis definition
        #             in app/analysis/rose-app.conf. If not, a validation error is thrown.
        ...
        for keys, sub_node in config_node.walk():
            ...
            if ascript is not None:
                ...
                if os.access(ascript, os.R_OK):
                    ...
                    self.add_report( 'template variables','app/analysis/rose-app.conf', f"{item}:{ascript}",
                                     "Not readable")
